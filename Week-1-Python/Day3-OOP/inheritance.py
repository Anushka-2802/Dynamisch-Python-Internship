#inheritance 
#used to inherit the properties or methods of parent class into child class
class Animal:
...
a=dog("Sheru") #obj for class dog
a.show() #inherit the method of Animal class
a.Eat()

#Method overriding : method has same name and parameters
class Animal:
   def bark(self):
...
class dog(Animal):
    def details(self,name,breed):
        super().__init__(name)
        self.breed=breed
        print(f"Name of Animal is {self.name} and breed is {self.breed}")
        
a=dog("Sheru")
a.details("Sheru","German Shepherd")
